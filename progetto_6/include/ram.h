...
/* Additional Includes for debug functions */
#include <stdio.h>
 ...
 * @param M la quantità di memoria voluta, espressa in KB (deve essere una
 * potenza di 2, maggiore o uguale a 1)
 ...
 * @return Il puntatore al nodo che può ospitare la quantità richiesta, oppure
 * NULL se non trovato
 */
...
/**
 * @brief calcola il numero di KB di memoria ancora liberi all'interno di una
 * struttura RAM
...
/**
 * @brief ricostruisce una struttura RAM a partire dalla sua rappresentazione
 * sotto forma di stringa creata da ram2str
 * @param str  la stringa che contiene la rappresentazione della RAM,
 * eventualmente vuota
...
/**
 * @brief cancella dallo heap la struttura dati di un nodo RAM e di tutti i suoi
 * figli
 * @param ram il nodo RAM da cancellare
 * @return Restituisce OK se la funzione ha effettivamente liberato della
 * memoria, NOK altrimenti
 */
